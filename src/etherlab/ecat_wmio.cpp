...
int EcatWmio::Init   (ec_master_t    *master_,
                    ...
                    uint16_t        position_)
{
    sc_io = ecrt_master_slave_config(master_, alias_, position_, WMIO);
    if(!sc_io)
    {
        fprintf(stderr, "Failed to get slave configuration.\n");
        return -1;
    }
    if (ecrt_slave_config_pdos(sc_io, EC_END, wmlan9252_io_syncs))
    {
...
{
    data.key0_1 = EC_READ_U16(domain1_pd_ + offset.keys);
...
{
    std::cout << "leds: " << (uint16_t)data.led0_7 << std::endl;
